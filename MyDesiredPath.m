% function [T, Xd, Thd] = DesiredPath(X_start,X_end,N)
function Xd = MyDesiredPath(X_start,X_end,N)
...
%% Line Tracking
xd = linspace(X_start(1),X_end(1),N);
yd = linspace(X_start(2),X_end(2),N);
zd = linspace(X_start(3),X_end(3),N);
Xd = [xd; yd; zd];
% Thd = zeros(3,length(Xd));
% T = 0::
